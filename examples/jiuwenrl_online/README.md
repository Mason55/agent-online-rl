...
如果通过 SSH 连接服务器（例如 `8.92.9.154`），用 SSH 隧道转发端口：

```bash
# 在本地机器执行（同时转发 Web 前端和 WebSocket）
ssh -L 5173:127.0.0.1:5173 -L 19000:127.0.0.1:19000 user@8.92.9.154
```

然后在本地浏览器打开 `http://localhost:5173` 即可。
...
**Q: 远程机器如何访问 Web 前端？**
SSH 隧道：`ssh -L 5173:127.0.0.1:5173 -L 19000:127.0.0.1:19000 user@HOST`，然后浏览器打开 `http://localhost:5173`。
